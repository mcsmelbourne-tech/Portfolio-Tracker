...
# Custom CSS to reduce font sizes of metrics and headers for a compact dashboard
st.markdown("""
    <style>
...
def load_data():
    expected_columns = ["ID", "Date", "Ticker", "Market", "Type", "Quantity", "Buy Price", "Sell Price", "Status"]
    try:
...
if not df.empty:
    ...
    for idx, row in df.iterrows():
        ...
        existing_market = row["Market"] if "Market" in df.columns and pd.notna(row["Market"]) else ""

        if "NS" in t_ticker or "BO" in t_ticker or existing_market == "India (INR)":
            market = "India (INR)"
            flag = "🇮🇳"
...
st.markdown("Multi-market tracker partitioned by USA, India, and Australian CFD assets.")
st.markdown("---")

# --- SEGREGATED CAPITAL METRICS DISPLAY WITH FLAGS ---
...
with st.expander("➕ Add New Trade", expanded=df.empty):
    with st.form("trade_form", clear_on_submit=True):
        ...
        with col1:
            ...
            ticker = st.text_input("Ticker Symbol (e.g., AAPL, RELIANCE.NS, BHP.AX)", "").upper()
        with col2:
            market_selection = st.selectbox("Market / Currency Group", ["USA (USD)", "India (INR)", "CFD (AUD)"])
            trade_type = st.selectbox("Type", ["Buy/Long", "Sell/Short"])
        with col3:
            quantity = st.number_input("Quantity", min_value=0.01, value=10.0, step=1.0)
            buy_price = st.number_input("Buy Price", min_value=0.01, value=100.0, step=0.1)

        status = st.selectbox("Status", ["Active", "Closed"])
        sell_price = 0.0
        if status == "Closed":
            sell_price = st.number_input("Sell Price", min_value=0.0, value=105.0, step=0.1)
        ...
        if submitted:
            if not ticker:
                ...
                raw_df = load_data()
                new_id = int(raw_df["ID"].max()) + 1 if not raw_df.empty and "ID" in raw_df.columns and pd.notna(raw_df["ID"].max()) else 1
...
if not df.empty:
    # Main Dashboard Tabs
    tab1, tab2, tab3 = st.tabs(["📋 Trade Ledger & Live Prices", "📊 Regional Analytics & Charts", "🗑️ Delete Trades"])
    ...
    with tab2:
        ...
        with chart_col1:
            ...
            if not india_df.empty:
                fig_ind = px.pie(india_df, names="Ticker", values="Current Value", hole=0.3)
        ...
        with chart_col2:
            ...
            if not usa_df.empty:
                fig_usa = px.pie(usa_df, names="Ticker", values="Current Value", hole=0.3)
        ...
        with chart_col3:
            ...
            if not cfd_df.empty:
                fig_cfd = px.pie(cfd_df, names="Ticker", values="Current Value", hole=0.3)
                st.plotly_chart(fig_cfd, use_container_width=True)
            else:
                st.info("No CFD trades.")

        st.markdown("---")
        st.subheader("Overall Profit / Loss per Ticker")
        fig_bar = px.bar(df, x="Ticker", y="Profit/Loss ($)", color="Profit/Loss ($)", color_continuous_scale=["red", "green"])
    ...
    st.info("No trades recorded yet. Use the **'➕ Add New Trade'** section above to get started!")
